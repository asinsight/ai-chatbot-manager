...
| **Platform M5** | ✅ 완료 (develop 대기) | `feat/feature_M5_workflows_logs` | `/workflows` + `/logs` + 메인 봇 strict + UX 정리. /workflows: Stage assignments dropdown ↔ `COMFYUI_WORKFLOW{,_HQ}` env, auto facts (node count / Σ steps / refiner+detailer) + admin description (`config/workflow_descriptions.json`), Form/Raw/Replace 3 탭 — Replace 시 `%prompt%` + `%negative_prompt%` placeholder 검증 강제. /logs: file picker (bot.log + dated archives) + tail/refresh/regex/download. 메인 봇: `bot.py` 빈 토큰/이름 시 SystemExit + platform pre-flight 422 + dashboard 경고 + /env required gate. UX: title "Chatbot Manager", Select 투명 fix, bot.log 라인 dedup. |
| **Post-M5** | 🔜 다음 | — | 별도 PR 후보 (video_models.json catalog, Prompt Guard 인증, SSE 로그 푸시, title-based prompt injection 등). 자세한 진행 상황은 [STATUS.md](STATUS.md) 참조 |
