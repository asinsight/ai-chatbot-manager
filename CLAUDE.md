...
| **Platform M4** | ✅ 완료 | `4621d27` | `/config` 3 탭 (sfw_scenes / pose_motion_presets / sfw_denylist) — master-detail + chips + Raw JSON fallback + zod 검증. profile_keys 는 `/prompts` 3번째 탭으로 이동 (LLM canonicalization config 라서). character_card_schema 는 `/characters` "View schema" Dialog 로 read-only viewer 분리 (편집 불가) + 한국어 description 영어로 번역. 4 pre-existing lint 에러 (npm run build 차단) 부수 fix. |
| **Platform M5** | ✅ 완료 | `b36338b` | `/workflows` + `/logs` + 메인 봇 strict + UX 정리. /workflows: Stage assignments dropdown ↔ `COMFYUI_WORKFLOW{,_HQ}` env, auto facts (node count / Σ steps / refiner+detailer) + admin description (`config/workflow_descriptions.json`), Form/Raw/Replace 3 탭 — Replace 시 `%prompt%` + `%negative_prompt%` placeholder 검증 강제. /logs: file picker (bot.log + dated archives) + tail/refresh/regex/download. 메인 봇: `bot.py` 빈 토큰/이름 시 SystemExit + platform pre-flight 422 + dashboard 경고 + /env required gate. UX: title "Chatbot Manager", Select 투명 fix, bot.log 라인 dedup. |
| **Post-M5 cleanup** | ✅ 완료 | `d1be949` + `aea64a4` | 보안 (사설 IP 제거 + GROK_BASE_URL 통일) + deploy 정리 (.service / install.sh / backup_db.sh 제거 + Korean 영어화 + prompt-guard/ 신설) + 봇 UX (TOS/privacy/consent 제거, Chatbot Manager rename, 📷 fixation 게이트 제거) + 워크플로우 (HQ NSFW 4 노드 strip + checkpoint dropdown auto-populate). |
| **Platform M6** | ✅ 완료 (develop 대기) | `feat/feature_M6_lorebook` | Lorebook editor — `world_info/char05.json` (Jiwon Han sample 6 entries) + `world_info/mapping.json` (`char_id → world_id`). `src/prompt.py` mapping 우선 + legacy fallback. `/lorebook` 페이지: Mapping card + World list (Add/Duplicate/Delete with WORLD_IN_USE 안내) + World editor (Test pane mirrors `_match_world_info()` + chips/textarea/position select). 4 API routes + `lib/lorebook{,-meta}.ts` (zod). |
| **Post-M6** | 🔜 다음 | — | 별도 PR 후보 (video_models.json catalog, Prompt Guard 인증, SSE 로그 푸시, title-based prompt injection 등). 자세한 진행 상황은 [STATUS.md](STATUS.md) 참조 |
